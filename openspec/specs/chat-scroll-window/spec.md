...
The message list SHALL automatically stick to the bottom during streaming when the user has not intentionally scrolled away, using hysteresis to avoid accidental disengagement.
...
- **WHEN** the user scrolls upward more than 150px from the bottom
...
- **GIVEN** auto-scroll is disengaged and the ↓ button is visible
- **AND** a conversation is receiving streaming tokens
- **WHEN** the user scrolls within 50px of the bottom
...
- **THEN** the list scrolls to the bottom to show the new user message
