# tool-file-ops Specification

## ADDED Requirements

### Requirement: read_file — 读取文件
...
- 图片文件（.png/.jpg/.gif/.webp）→ 尝试读取为图片
- PDF 文件 → 支持 pages 参数指定页面范围

search_hint: `"read file contents with optional line range and image/pdf support"`
aliases: `["read", "cat"]`

### Requirement: write_file — 创建/覆盖文件
...
search_hint: `"create or overwrite a file at path with content"`
aliases: `["write", "create_file"]`

### Requirement: edit_file — 精确字符串替换
...
search_hint: `"perform exact string replacements in existing file"`
aliases: `["edit"]`

### Requirement: validate_input 行为
...
### Requirement: get_path 行为

三者均从 `file_path` 参数中提取操作目标路径。

### Requirement: user_facing_name 行为
