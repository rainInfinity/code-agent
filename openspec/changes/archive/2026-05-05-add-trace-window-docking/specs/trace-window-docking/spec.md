...
贴靠模式下，Trace 窗口 SHALL 跟随主窗口移动和尺寸变化，并 SHALL 保持与主窗口相同的高度。
...
#### Scenario: 主窗口最大化时贴靠到内侧
- **GIVEN** Trace 窗口处于右侧贴靠模式
- **WHEN** 主窗口最大化
- **THEN** Trace 窗口 SHALL 贴靠到主窗口内侧右边缘
- **AND** Trace 窗口 SHALL 保持在主窗口前方可见
...
贴靠模式下，Trace 窗口宽度 SHALL 可由用户拖拽调整，并 SHALL 在最小宽度和最大宽度之间保存为独立的贴靠宽度状态。

#### Scenario: 用户调整贴靠宽度
- **GIVEN** Trace 窗口处于右侧贴靠模式
- **WHEN** 用户拖拽 Trace 窗口边缘调整宽度
- **THEN** Trace 窗口宽度 SHALL 被限制在允许的最小值和最大值之间
- **AND** 系统 SHALL 保存新的贴靠宽度
- **AND** Trace 窗口 SHALL 保持与主窗口右边缘贴靠
...
贴靠模式下，Trace 窗口 SHALL 禁用最大化操作，并 SHALL 将标题栏拖拽解释为退出贴靠的用户意图。
...
#### Scenario: 拖拽标题栏退出贴靠
- **GIVEN** Trace 窗口处于右侧贴靠模式
- **WHEN** 用户从 Trace 标题栏开始拖拽窗口
- **THEN** Trace 窗口 SHALL 退出贴靠模式
