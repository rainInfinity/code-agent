...
```
输入: [read_file, grep, write_file, glob]
       safe✓       safe✓  unsafe✗      safe✓

输出: [Batch { conc: true,  [read_file] },      // 安全，开启并发组
       Batch { conc: false, [grep] },            // 注意：grep 和 read_file 都是 safe，但 write_file 中断了连续 → grep 归入前一组
       
修正后的正确输出:
       Batch { conc: true,  [read_file, grep] }, // 连续 safe → 合并
       Batch { conc: false, [write_file] },       // unsafe → 独立串行
       Batch { conc: true,  [glob] }]             // 新 safe 组开始
```
